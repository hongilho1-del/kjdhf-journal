...
values
  ...
  ('21000000-0000-0000-0000-000000000003', 'authenticated', 'authenticated', 'workflow.reviewer3@example.invalid', crypt('Test-password-1!', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{"full_name":"통합테스트 심사위원3"}', now(), now()),
...
update public.profiles set role = 'REVIEWER' where id in ('21000000-0000-0000-0000-000000000001', '21000000-0000-0000-0000-000000000002', '21000000-0000-0000-0000-000000000003');
...
-- 5~6. 편집위원이 접수·형식검토 후 3명의 심사위원을 배정한다.
reset role;
...
select public.assign_reviewer('51000000-0000-0000-0000-000000000001', '21000000-0000-0000-0000-000000000003', now() + interval '14 days');
...
set local role authenticated;
select public.respond_to_review_assignment((select id from public.reviewer_assignments where reviewer_id = auth.uid() and round_no = 1), true, null);
select public.submit_review((select id from public.reviewer_assignments where reviewer_id = auth.uid() and round_no = 1), 'ACCEPT_WITH_REVISIONS', '분석 결과를 조금 더 설명해 주세요.', '수정후게재를 권고합니다.');
...
select public.assign_reviewer('51000000-0000-0000-0000-000000000001', '21000000-0000-0000-0000-000000000003', now() + interval '7 days');
...
begin
  ...
  if (select count(*) from public.reviewer_assignments where manuscript_id = '51000000-0000-0000-0000-000000000001' and round_no = 1) <> 3 then raise exception 'Three-reviewer assignment failed'; end if;
  if (select count(*) from public.reviews r join public.reviewer_assignments ra on ra.id = r.assignment_id where ra.manuscript_id = '51000000-0000-0000-0000-000000000001' and r.status = 'SUBMITTED') <> 3 then raise exception 'Review submission failed'; end if;
end $$;
