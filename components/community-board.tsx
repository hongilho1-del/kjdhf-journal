...
export function CommunityBoard({
  ...
  const loadPosts = useCallback(async () => {
    ...
    if (error) {
      ...
      const nextPosts = (data ?? []).map(normalizeBoardPostIdentity);
      setPosts(nextPosts);
    ...
    <section className="community-page">
      ...
      <div className="shell community-layout">
        <aside className="community-side-nav">
          ...
          <button className={category === "EVENT" ? "active" : ""} type="button" onClick={() => onCategoryChange("EVENT")}>학술대회 <span>›</span></button>
        </aside>
        <div className="community-content">
          {selected ? (
            <article className="board-detail">
              <div className="board-title-row">
                <span>{selected.category === "NOTICE" ? "공지" : "학술대회"}</span>
                <h2>{selected.title}</h2>
                <time>{formatDate(selected.published_at ?? selected.created_at)}</time>
              </div>
              {selected.category === "EVENT" && (
                <dl className="event-metadata">
                  <div><dt>학술대회 일시</dt><dd>{formatDate(selected.event_start_at)}{selected.event_end_at ? ` ~ ${formatDate(selected.event_end_at)}` : ""}</dd></div>
              ...
              {loading ? <div className="empty-state">게시물을 불러오는 중입니다.</div> : filteredPosts.length === 0 ? <div className="empty-state"><strong>등록된 게시물이 없습니다.</strong></div> : (
                <div className="board-table-wrap">
                  <table className="board-table">
                    <thead><tr><th>번호</th><th>제목</th>{category === "EVENT" && <th>학술대회일</th>}<th>등록일</th></tr></thead>
