...
import { getErrorMessage, ROLE_LABELS, type Profile } from "@/lib/journal";
import { getSupabaseClient } from "@/lib/supabase/client";

export function ProfilePanel({ profile, onSaved }: { profile: Profile; onSaved: () => Promise<void> }) {
  ...
  return (
    <section className="workspace-card profile-card">
      <div className="card-heading"><div><p>MY PROFILE</p><h2>개인정보 수정</h2></div><span className="role-chip">{ROLE_LABELS[profile.role]}</span></div>
      <form className="form-grid" onSubmit={handleSubmit}>
        ...
        {profile.role === "REVIEWER" && <label className="wide">심사 전문분야<textarea name="reviewerBio" defaultValue={profile.reviewer_bio ?? ""} rows={4} /></label>}
